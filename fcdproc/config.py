...
# -*- coding: utf-8 -*-
"""
Created on Tue Mar 30 08:15:11 2021
...
r"""
A Python module to maintain unique, run-wide *FCDproc* settings.
This module implements the memory structures to keep a consistent, singleton config.
Settings are passed across processes via filesystem, and a copy of the settings for
each run and subject is left under
``<fcdproc_dir>/sub-<participant_id>/log/<run_unique_id>/fcdproc.toml``.
Settings are stored using :abbr:`ToML (Tom's Markup Language)`.
The module has a :py:func:`~fcdproc.config.to_filename` function to allow writting out
the settings to hard disk in *ToML* format, which looks like:
.. literalinclude:: ../fcdproc/data/tests/config.toml
   :language: toml
   :name: fcdproc.toml
   :caption: **Example file representation of fMRIPrep settings**.
This config file is used to pass the settings across processes,
using the :py:func:`~fcdproc.config.load` function.
...
.. code-block:: Python
    ...
    config_file = config.execution.work_dir / '.fcdproc.toml'
...
if not (
    ...
    os.getenv("FCDPROC_DEV", "0").lower() in ("1", "on", "true", "y", "yes"),
...
class environment(_Config):
    ...
    execution environment, nipype and FCDproc* versions, etc.).
...
class execution(_Config):
    ...
    #"""A path where anatomical derivatives are found."""
    controls = None
    """List of control subjects """
    patient_positive = None
    """List of patient positives """
    patient_negative = None
    """List of patient_negative """
    ...
    """Debug mode(s)."""
    fcdproc_dir = None
    """Root of fcdproc BIDS Derivatives dataset. Depends on output_layout."""
    ...
    notrack = False
    """Do not monitor *FCDproc* using Sentry.io."""
    output_dir = None
...
class workflow(_Config):
    ...
    analysis_mode = "preprocess"
    """type of analysis can be preprocessing, modeling, or detecting"""
    ...
    ignore = None
    """Ignore particular steps for *FCDproc*."""
    longitudinal = False
    ...
    *FCDproc* will run brain extraction of the T1w)."""
    spaces = None
